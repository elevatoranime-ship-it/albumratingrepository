...
${suffix}`;
  ...
  var CLOUD = !DEMO && Boolean(cfg && cfg.supabaseUrl && cfg.supabaseAnonKey);
  var ALLOWED_USERS = !DEMO && cfg && cfg.allowedUsers && cfg.allowedUsers.length ? cfg.allowedUsers : DEFAULT_USERS;
  var DEMO_PASSWORD = DEMO ? "demo" : cfg?.demoPassword ?? "demo";
  var sb = null;
  function getSB() {
  ...
  var avatarStyle = (url) => `background-image:url('${url.replace(/'/g, "%27")}');background-size:cover;background-position:center`;
  function avatarMarkup(info, cls) {
  ...
  var fmt = (n) => String(parseFloat(n.toFixed(2)));
  var round2 = (n) => Math.round(n * 100) / 100;
  var COHESION_OPTIONS = [
  ...
  var parentIdsOf = (a) => a.parentIds ?? (a.parentId ? [a.parentId] : []);
  var parentsOf = (a) => parentIdsOf(a).map((id) => albums.find((x) => x.id === id && x.kind === "album")).filter((x) => Boolean(x));
  var parentOf = (a) => parentsOf(a)[0];
  ...
  var card = q("#card");
  var viewLogin = q("#view-login");
  var viewHome = q("#view-home");
  var viewAlbum = q("#view-album");
  var viewAdd = q("#view-add");
  ...
  var profileBtn = q("#profile-btn");
  var homeAvatar = q("#home-avatar");
  var success = q("#success");
  var brandTitle = q("#brand-title");
  var loginForm = q("#login-form");
  var emailInput = q("#email-input");
  var passwordInput = q("#password");
  var password2Input = q("#password2");
  var confirmField = q("#confirm-field");
  var toggleBtn = q("#toggle-password");
  var formError = q("#form-error");
  var submitBtn = q("#submit-btn");
  var submitLabel = q("#submit-label");
  var authSwitchHint = q("#auth-switch-hint");
  var authSwitch = q("#auth-switch");
  ...
  var avAvg = q("#av-avg");
  var avTracksCount = q("#av-tracks-count");
  var tracksLockBtn = q("#tracks-lock-btn");
  var tracksLockLabel = q("#tracks-lock-label");
  ...
  var trackList = q("#track-list");
  var avImpact = q("#av-impact");
  var avChips = q("#av-chips");
  var avConfirmState = q("#av-confirm-state");
  var cohesionControl = q("#cohesion-control");
  var typeControl = q("#type-control");
  var albumDeleteBtn = q("#album-delete-btn");
  var artistBack = q("#artist-back");
  var artistName = q("#artist-name");
  var artistScore = q("#artist-score");
  ...
  var profUpload = q("#prof-upload");
  var profRemove = q("#prof-remove");
  var profFile = q("#prof-file");
  ...
  var confirmOk = q("#confirm-ok");
  var confirmCancel = q("#confirm-cancel");
  ...
  var coverPick = q("#cover-pick");
  var coverFile = q("#cover-file");
  var coverImg = q("#cover-img");
  var coverRemove = q("#cover-remove");
  ...
  logoutBtn.addEventListener("click", () => {
    void (async () => {
      ...
      viewHome.querySelectorAll(".album").forEach((el) => {
  ...
  function updateRatingDisplays(changedTrackId) {
    ...
    if (changedTrackId) {
      const el = trackList.querySelector(`[data-tid="${changedTrackId}"]`);
      if (el) tweenText(el, trackScoreOf(changedTrackId));
    } else {
      for (const t of list) {
        const el = trackList.querySelector(`[data-tid="${t.id}"]`);
  ...
  function renderImpact() {
    ...
    for (const r of rows) {
      ...
      <span class="av__chip-sub">${label} \xB7 ${r.n} \u0438\u0437 ${list.length}</span>`;
      chip.title = `${r.username}: ${r.mean === null ? "\u043D\u0435\u0442 \u043E\u0446\u0435\u043D\u043E\u043A" : fmt(r.mean) + " \u0432 \u0441\u0440\u0435\u0434\u043D\u0435\u043C"} \xB7 ${r.n}/${list.length} \u0442\u0440\u0435\u043A\u043E\u0432`;
  ...
  function renderConfirmState() {
    ...
    avConfirmState.innerHTML = final ? `${CHECK_SVG}<span>\u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0436\u0434\u0451\u043D</span>` : `${PENDING_SVG}<span>\u043D\u0435 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0436\u0434\u0451\u043D</span>`;
  }
  var CHEVRON_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>';
  function finSelectHTML(kind, options) {
    const opts = options.map((o, i) => `<li><button type="button" class="fin-select__option" role="option" data-value="${esc(o.value)}" style="--d:${i * 28}ms">${esc(o.label)}</button></li>`).join("");
  ...
  function finalBadgeHTML(text, animate) {
    const icon = animate ? `<span class="final__icon">
       ...
       </span>` : `<span class="final__icon">${LOCK_SVG}</span>`;
  ...
  function closeAllFinSelects() {
    document.querySelectorAll(".fin-select.is-open").forEach((el) => {
      el.classList.remove("is-open");
      const tr = el.querySelector(".fin-select__trigger");
  ...
  viewAlbum.addEventListener("click", (e) => {
    ...
    if (trigger) {
      const root = trigger.closest(".fin-select");
      if (!root) return;
  ...
  async function onFinalizePick(kind, value, label) {
    ...
    const ok = await openConfirm(
      title,
      `\xAB${label}\xBB \u2014 \u0432\u044B\u0431\u043E\u0440 \u0441\u0442\u0430\u043D\u0435\u0442 \u043E\u043A\u043E\u043D\u0447\u0430\u0442\u0435\u043B\u044C\u043D\u044B\u043C \u0438 \u0435\u0433\u043E \u043D\u0435\u043B\u044C\u0437\u044F \u0431\u0443\u0434\u0435\u0442 \u0438\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u043D\u0438\u043A\u043E\u043C\u0443 \u0438\u0437 \u0432\u0430\u0441.`
    );
    if (!ok) {
      renderFinalize();
      return;
  ...
  var UP_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 15l6-6 6 6"/></svg>';
  var DOWN_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>';
  var DEL_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M5 5l14 14M19 5L5 19"/></svg>';
  var LOCK_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="10" rx="2.5"/><path d="M8 10.5V7a4 4 0 0 1 8 0v3.5"/></svg>';
  var UNLOCK_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="10" rx="2.5"/><path d="M8 10.5V7a4 4 0 0 1 7.8-1.4"/></svg>';
  ...
  function updateFeatList() {
    ...
    if (!matches.length && q0) {
      ...
      btn.innerHTML = `<span class="combo__name">\u043D\u043E\u0432\u044B\u0439 \u0430\u0440\u0442\u0438\u0441\u0442: \xAB${esc(trackFeatInput.value.trim())}\xBB</span>`;
      btn.addEventListener("pointerdown", (e) => {
        e.preventDefault();
        pickFeat(trackFeatInput.value.trim());
      });
      li.appendChild(btn);
  ...
  document.addEventListener("click", (e) => {
    const combo = document.querySelector(".av__feat-combo");
    if (combo && !combo.contains(e.target)) trackFeatList.hidden = true;
  });
  ...
  async function toggleTrackLock(id) {
    if (!isAdmin()) return;
    const t = tracks.find((x) => x.id === id);
    if (!t) return;
    t.locked = !t.locked;
  ...
  async function toggleTracksLock() {
    if (!isAdmin()) return;
    const al = currentAlbum();
    if (!al) return;
  ...
  async function persistTrackOrder() {
    const list = tracks.filter((t) => t.albumId === currentAlbumId).sort((a, b) => a.position - b.position);
    if (CLOUD) {
  ...
  trackList.addEventListener("click", (e) => {
    ...
    if (confirmBtn) {
      ...
      const cli = confirmBtn.closest(".track");
  ...
  avArtist.addEventListener("click", (e) => {
    const a = e.target.closest(".av__artist-link");
    if (!a) return;
    e.preventDefault();
    const name = a.dataset.artist;
    if (name) void openArtist(name);
  });
  q("#albums").addEventListener("click", (e) => {
    const a = e.target.closest(".album__artist-link");
    if (!a) return;
    e.preventDefault();
    const name = a.dataset.artist;
    if (name) void openArtist(name);
  ...
  async function saveNickname() {
    ...
    profError.classList.remove("is-visible");
    if (!name) {
      profError.textContent = "\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u043D\u0438\u043A";
  ...
  async function handleDeleteAlbum() {
    ...
    const ok = await openConfirm(
      "\u0423\u0434\u0430\u043B\u0438\u0442\u044C \u0430\u043B\u044C\u0431\u043E\u043C?",
      `\u0410\u043B\u044C\u0431\u043E\u043C <b>\xAB${esc(al.title)}\xBB</b> \u2014 ${esc(al.artist)} \u0431\u0443\u0434\u0435\u0442 \u0443\u0434\u0430\u043B\u0451\u043D <b>\u043D\u0430\u0432\u0441\u0435\u0433\u0434\u0430</b> \u0432\u043C\u0435\u0441\u0442\u0435 \u0441\u043E \u0432\u0441\u0435\u043C\u0438 \u0442\u0440\u0435\u043A\u0430\u043C\u0438 \u0438 \u043E\u0446\u0435\u043D\u043A\u0430\u043C\u0438. \u042D\u0442\u043E \u0434\u0435\u0439\u0441\u0442\u0432\u0438\u0435 \u043D\u0435\u043B\u044C\u0437\u044F \u043E\u0442\u043C\u0435\u043D\u0438\u0442\u044C.`,
      true
    );
    if (!ok) return;
    try {
      if (CLOUD) {
        ...
        albums = albums.filter((a) => a.id !== al.id).map((a) => {
      ...
      if (wasLast) {
        await crumbleCover();
        renderAlbums();
        await swapTo(viewAlbum, viewHome, () => {
          viewHome.scrollTop = 0;
        });
      }
      toast("\u0410\u043B\u044C\u0431\u043E\u043C \u0443\u0434\u0430\u043B\u0451\u043D");
  ...
  function singleOriginText(s) {
    ...
    const count = new Set(tracks.filter((t) => t.singleId === s.id && albumIds.has(t.albumId)).map((t) => t.albumId)).size;
    if (!count) return "";
    const singular = count % 10 === 1 && count % 100 !== 11;
    return `\u041E\u0442\u043C\u0435\u0447\u0435\u043D \u0441\u0438\u043D\u0433\u043B\u043E\u043C \u0432 ${count} ${singular ? "\u0430\u043B\u044C\u0431\u043E\u043C\u0435" : "\u0430\u043B\u044C\u0431\u043E\u043C\u0430\u0445"}`;
  }
  function parentLinksHtml(s, onPage = false) {
    const parents = parentsOf(s);
    if (!parents.length) return onPage ? "\u0441\u0438\u043D\u0433\u043B \u0432\u043D\u0435 \u0430\u043B\u044C\u0431\u043E\u043C\u0430" : "";
    const link = (a) => `<a href="#album-${encodeURIComponent(a.id)}" class="sv__parent-link" data-album="${esc(a.id)}">\xAB${esc(a.title)}\xBB</a>`;
  ...
  function renderSingleParents(s) {
  ...
  async function toggleSingleParents(more) {
    ...
    const list = more.parentElement.querySelector(".parent-list");
    const intro = svParentLabel.querySelector(".parent-intro");
  ...
  function handleParentClick(e) {
    const target = e.target;
    const more = target.closest(".parent-more");
    if (more) {
      void toggleSingleParents(more);
    } else {
      const link = target.closest(".sv__parent-link");
  ...
  function mirrorSingleToTracks(singleId, immediate = false, exceptTrackId) {
    if (!currentUser) return;
  ...
  function albumInputLabel(a) {
    return albumsOnly().filter((x) => x.title.toLowerCase() === a.title.toLowerCase()).length > 1 ? `${a.title} \u2014 ${a.artist} (${a.year}) [${a.id}]` : a.title;
  }
  function quoteAlbumToken(token) {
    return /[,"\n]/.test(token) ? `"${token.replace(/"/g, '""')}"` : token;
  }
  function singleLinkErrorMessage(err) {
    const message = messageOf(err);
    return /parent_album_ids/.test(message) ? "\u0414\u043B\u044F \u043D\u0435\u0441\u043A\u043E\u043B\u044C\u043A\u0438\u0445 \u0430\u043B\u044C\u0431\u043E\u043C\u043E\u0432 \u043D\u0443\u0436\u043D\u0430 \u043C\u0438\u0433\u0440\u0430\u0446\u0438\u044F \u0431\u0430\u0437\u044B: \u0432\u044B\u043F\u043E\u043B\u043D\u0438\u0442\u0435 migrate.sql \u0432 Supabase" : message;
  }
  function resolveAlbumTokens(value) {
    const ids = [];
    for (const token of albumTokens(value).filter(Boolean)) {
      const matches = albumsOnly().filter((a) => albumInputLabel(a).toLowerCase() === token.toLowerCase() || a.title.toLowerCase() === token.toLowerCase());
      if (matches.length !== 1) throw new Error(matches.length ? `\u041D\u0430\u0437\u0432\u0430\u043D\u0438\u0435 \xAB${token}\xBB \u043D\u0435\u043E\u0434\u043D\u043E\u0437\u043D\u0430\u0447\u043D\u043E \u2014 \u0432\u044B\u0431\u0435\u0440\u0438\u0442\u0435 \u0430\u043B\u044C\u0431\u043E\u043C \u0438\u0437 \u043F\u043E\u0434\u0441\u043A\u0430\u0437\u043E\u043A` : `\u0410\u043B\u044C\u0431\u043E\u043C \xAB${token}\xBB \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D \u2014 \u0432\u044B\u0431\u0435\u0440\u0438\u0442\u0435 \u0438\u0437 \u043F\u043E\u0434\u0441\u043A\u0430\u0437\u043E\u043A \u0438\u043B\u0438 \u043E\u0447\u0438\u0441\u0442\u0438\u0442\u0435 \u043F\u043E\u043B\u0435`);
      if (!ids.includes(matches[0].id)) ids.push(matches[0].id);
    }
    return ids;
  }
  function updateAlbumSuggestions(input, list, picked) {
    const tokens = albumTokens(input.value);
    const query = tokens.pop().toLowerCase();
    const selected = tokens.map((t) => t.toLowerCase());
    const matches = albumsOnly().filter((a) => !selected.includes(albumInputLabel(a).toLowerCase()) && (!query || a.title.toLowerCase().includes(query) || a.artist.toLowerCase().includes(query))).slice(0, 8);
    list.innerHTML = "";
  ...
  async function saveSingleLink(parentIds) {
    ...
    if (JSON.stringify(parentIdsOf(single)) === JSON.stringify(parentIds)) {
      closeSingleLinkEditor();
      return;
    }
  ...
  async function attachSingleTracks(singleId, parentIds) {
    const warnings = [];
  ...
  function updateArtistList() {
    ...
    if (matches.length === 0 && q0) {
      ...
      btn.type = "button";
      btn.className = "combo__item combo__item--new";
      btn.innerHTML = `<span class="combo__name">\u043D\u043E\u0432\u044B\u0439 \u0430\u0440\u0442\u0438\u0441\u0442: \xAB${esc(artistInput.value.trim())}\xBB</span>`;
      btn.addEventListener("pointerdown", (e) => {
        e.preventDefault();
  ...
  document.addEventListener("click", (e) => {
    if (!artistBox.contains(e.target)) artistList.hidden = true;
  });
  ...
  async function tryCoverUrl(url) {
    if (!/^https?:\/\//i.test(url)) {
  ...
  async function handleAdd() {
    ...
    if (!artistRaw) err = "\u0423\u043A\u0430\u0436\u0438\u0442\u0435 \u0430\u0440\u0442\u0438\u0441\u0442\u0430";
      ...
      if (y < 1900 || y > (/* @__PURE__ */ new Date()).getFullYear()) err = `\u0413\u043E\u0434 \u2014 \u043E\u0442 1900 \u0434\u043E ${(/* @__PURE__ */ new Date()).getFullYear()}`;
    }
    if (err) {
      showAddError(err);
      shakeEl(addPanel);
  ...
  function initTilt() {
    ...
    function applyTilt() {
      ...
      tx += (cx - tx) * 0.12;
      ty += (cy - ty) * 0.12;
      card.style.transform = `perspective(1100px) rotateX(${(-ty * 3.4).toFixed(2)}deg) rotateY(${(tx * 4.2).toFixed(2)}deg)`;
